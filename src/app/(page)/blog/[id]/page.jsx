...
import { data } from "@/app/Components/GetBlog.jsx";

export async function generateMetadata({ params }) {
  let { id } = await params;
  let thisBlog = data?.find((blog) => blog?.id === Number(id));
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-3xl shadow-lg mt-10 mb-20">
      ...
      <div className="flex flex-col items-center gap-6 text-center">
        ...
        />
        <p className="text-gray-700 text-lg leading-relaxed border border-gray-200 rounded-lg px-4 py-2 shadow-inner bg-gray-50">
          {thisBlog?.desc}
        </p>
      </div>
    </div>
